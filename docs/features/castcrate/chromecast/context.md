...
- 🔗 **Cast session has no heartbeat** — ~~spans chromecast ↔ cast-controls ↔ player-buffer-ux — add 30s heartbeat + WS event.~~ **Resolved 2026-08-09** — 30s `player.getStatus` probe + 5s manual timeout + N=2 failures + `cast:disconnected` WS event landed same session. See Session notes below.
...
- 💳 **Single-session model unprotected; no timeout; registered WS unused** — ~~two concurrent `POST /api/cast/play` race the Map; device-off-overnight leaves session forever.~~ **Partially resolved 2026-08-09**: WS now carries cast events; device-off-overnight now flips to `disconnected` after ~60s. Still open: two-concurrent-play race (needs a mutex or "cast already active" rejection).
