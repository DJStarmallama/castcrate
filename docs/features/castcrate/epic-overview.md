...
| 14 | subtitles | 🟡 In Progress | 12/20 (60%) | Subtitle sourcing & rendering (OpenSubtitles fallback added 2026-08-09) |
...
- [x] ✅ 🔗 **Indexer fallback wiring duplicated per adapter, error shapes drift** — yts-streaming ↔ knaben-fallback ↔ torrentday-indexer ↔ stremio-addon-source — **Resolved 2026-08-09** (commit `6a67bde`): extracted `IndexerAdapter` + `runFallback` to `apps/server/src/lib/indexers.ts`; all adapters register through the common interface; `routes/torrents.ts` builds `movieChain`/`episodeChain`/`seasonPackChain` at module init. Response `tried[]` shape stayed wire-compatible. +9 unit tests locking in fallback semantics.
...
- [x] ✅ 🔗 **Cast session has no heartbeat — stale sessions live forever** — chromecast ↔ cast-controls ↔ player-buffer-ux — **Resolved 2026-08-09** (commit `212d4d7`): 30s `player.getStatus` probe (with a mandatory 5s manual `Promise` timeout — castv2-client's request/response has no built-in timeout, so a hung socket would never advance the failure counter). After 2 consecutive failures (~60s), session flips to a new `"disconnected"` state, broadcasts `cast:disconnected` over the WS bridge, tears down the local client, stops probing. `CastControls.tsx` renders an amber banner + working Stop; polling freezes on disconnect; server short-circuits Stop for disconnected sessions to avoid hanging on the dead socket.
- [x] ✅ 🔗 **Subtitle picker is a no-op during active cast** — subtitles ↔ cast-controls ↔ player-buffer-ux — **Resolved 2026-08-09** (commit `678a7e0`): enumerate ALL subtitle tracks upfront on cast start (was: only the picker-selected one), new route `POST /api/cast/sessions/:id/tracks` calls `player.media.sessionRequest({ type: "EDIT_TRACKS_INFO", activeTrackIds })` for hot-swap. `DefaultMediaReceiver` doesn't proxy `editTracksInfo` — had to reach into `MediaController.sessionRequest` directly. Track IDs = `subtitle.index + 1` (0 reserved because some Chromecast firmware treats it as "no active track"). HTTP-stream (Stremio debrid) cast sessions still can't hot-swap.
...
- [x] ✅ 🔗 **Torrent-lifecycle idempotency was implicit and untested — the recent crashes proved it** — yts-streaming ↔ library-settings ↔ transcoding ↔ player-buffer-ux — **Resolved 2026-08-09** (commit `dafcfce`): added `services/__tests__/torrent.test.ts` with 4 vitest cases: idempotency (double-remove), meta cleanup, `destroyStore` forwarding, non-idempotent rejection propagation. Also wired `torrent.once("close", …)` in `startTorrent` to clean `meta` map on external teardown (previously only `removeTorrent` cleared it).
...
- [ ] 🔗 **Subtitle track has no fallback if torrent disappears mid-cast** — subtitles ↔ chromecast ↔ player-buffer-ux — Chromecast is handed a `/stream/:hash/subtitles/:idx` URL; if the user removes the torrent the receiver silently loses the track. Warn in the picker when casting; ideally pin the underlying file until session ends. **Comment now in `routes/cast.ts`** flagging this for the next pass.
...
- [x] ✅ 🔗 **Buffer overlay has no formal state machine — logic scattered across 3+ components** — player-buffer-ux ↔ yts-streaming ↔ transcoding — **Resolved 2026-08-09** (commit `ba8fc84`): extracted `useBufferState()` reducer to `apps/web/src/hooks/useBufferState.ts`. `BufferState = "idle" | "initial-buffer" | "buffering" | "playing" | "initial-stalled" | "stalled" | "error"` (two states beyond starting five to preserve pre-refactor behavior — `initial-buffer` and `initial-stalled` for pre-`canplay` distinct transitions). `DEAD_SWARM_THRESHOLD_MS` currently 10s; flip to 30s when Phase 2/3 CTA lands. Behavior-preserving refactor — Phase 6 overlay dismiss still works.
...
- [x] ✅ 💳 **Phases A–D still zero-progress despite epic being in production** — hardening — **Phase B resolved 2026-08-09** (commit `dafcfce`): B1/B3/B5 verified already implemented; B2 (ffmpeg exit-await, replacing the flat 1.5s sleep with per-process 2s SIGKILL fallback), B4 (`STREAM_FIRST_BYTE_TIMEOUT_MS` envified to 60s default, applied to both `/stream/:hash` and `/stream/:hash/transcoded`), B6 (meta-map cleanup on external torrent close), B7 (removeTorrent idempotency test) landed. Tests: 218 → 222 (+4). **Phases A/C/D still open.**
...
- [x] ✅ 💳 **History writes non-atomic; `meta` map can leak; cast-only sessions leave no trace** — library-settings — **Resolved 2026-08-09** in hardening Phase B (commit `dafcfce`): atomic history writes verified already implemented (temp + rename); `meta` map now cleaned on `torrent.on("close", …)`; cast-start history append + dedup on removal via `historyId`.
...
- [~] 💳 **No encoding detection; language guessing is heuristic; subtitles aren't priority-bumped in WebTorrent** — subtitles — **Partially resolved 2026-08-09** by adding OpenSubtitles as a fallback source (`feat(subtitles): OpenSubtitles fallback source`): OS-sourced subs come back as properly-tagged UTF-8 SRTs with explicit ISO-639 language codes, sidestepping both the encoding-detection and language-guessing heuristics for that branch. **Still open** for torrent-embedded subs — non-UTF-8 SRTs from private trackers still render as garbage; picker shows "No subtitles" until WebTorrent's scheduler reaches the `.srt` file. Add `chardet`+`iconv`; call `file.select(1)` on chosen subtitle to bump priority.
- [ ] 💳 **Overlay layering Phase 6 landed, but Phases 2/3 (buffer-to-N%, dead-swarm CTA) still spec-only** — player-buffer-ux — Phase 6 shipped, and **the pre-req `useBufferState()` reducer landed 2026-08-09** (commit `ba8fc84`). Phases 2/3 are now unblocked to add new states + events on top of the reducer. **Still open: Phases 2 and 3 themselves.**
- [ ] 💳 **Single-session model unprotected; no timeout; registered WS unused** — chromecast — **Partially resolved 2026-08-09** (commit `212d4d7`): device-off-overnight now flips to `disconnected` after ~60s heartbeat failure; WS bridge is now actively carrying cast events (`cast:status`, `cast:closed`, `cast:disconnected`). **Still open:** two-concurrent-play race on the Map (needs a mutex or "cast already active" rejection).
...
All five top-priority items from the original review landed in a single 5-agent parallel session on 2026-08-09:

1. ✅ Hardening Phase B — atomic history + ffmpeg exit-await + stream timeouts + meta cleanup + idempotency test (`dafcfce`).
2. ✅ `useBufferState()` reducer extracted (`ba8fc84`) — unblocks Phases 2/3.
3. ✅ `IndexerAdapter` + `runFallback` extracted (`6a67bde`) — kills 5 cross-feature findings.
4. ✅ Cast session heartbeat + `cast:disconnected` WS event (`212d4d7`).
5. ✅ Subtitle hot-swap during active cast (`678a7e0`) — `EDIT_TRACKS_INFO` via `MediaController.sessionRequest`.

**Next queue (all still open):**

- **Stremio HTTP-stream sessions bypass history** — decide synthetic-id vs explicit-tier UX.
- **`mkdirSync` at module-load** — move behind first-use + surface path-writability in `/api/system/check`.
- **Two-concurrent-play race** on the cast session Map — mutex or "cast already active" rejection.
- **Subtitle track fallback if torrent disappears mid-cast** — warn or pin the file.
- **Transcoder fallback if ffmpeg dies mid-stream** — 502 before first byte for client retry.
- **`FallbackChain` proxy-cache-suffix helper** — export `getCacheKeySuffix()` so adapters don't manage the suffix by hand.
- **`hardening` Phases A/C/D** — env-var docs, UX polish, test coverage + CI.
- **Player-buffer-ux Phases 2/3** — buffer-to-N% dialog + dead-swarm CTA.
